...
].filter((row) => !['property-spaces', 'property-media', 'property-promotions'].includes(row[0])).map((row, index) => module(row[0], row[1], row[2], 'landlord', { icon: row[3], path: row[4], minTier: row[5], sortOrder: (index + 1) * 10, subscription: 'landlord', mobilePrimary: ['dashboard','properties','applications','messages','documents'].includes(row[0]) }));
...
const RESOURCE_ACCESS = Object.freeze({
  ...
  // Landlords hire from the public /surveyors directory. Job posting and
  // proposal comparison remain available only to Surveyors/admins for the
  // legacy marketplace compatibility path.
  'survey-jobs': { admin: ADMIN_ACTIONS, surveyor: SURVEYOR_ACTIONS },
