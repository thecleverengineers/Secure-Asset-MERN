...
export default function AppShell() {
  ...
  const drawerContent = (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
      <Box sx={{ px: collapsed && !isMobile ? 1.2 : 2, pt: 2, pb: collapsed && !isMobile ? 1.4 : 1.6 }}>
        ...
        {(!collapsed || isMobile) && <Box sx={{ mt: 2, p: 1.35, borderRadius: design.borders.navigationRadius, bgcolor: 'action.hover', border: `${design.borders.width}px ${design.borders.style}`, borderColor: 'divider' }}>
              ...
              return <Tooltip key={item.key} title={collapsed && !isMobile ? item.label : ''} placement="right">
                <ListItemButton
                  ...
                  sx={{
                    ...
                    bgcolor: active ? 'primary.main' : 'transparent', color: active ? 'primary.contrastText' : 'text.secondary',
                    '&::before': active ? { content: '""', position: 'absolute', left: 0, top: 10, bottom: 10, width: 3, borderRadius: 2, bgcolor: 'secondary.light' } : undefined,
                    '&:hover': { bgcolor: active ? 'primary.dark' : 'action.hover', color: active ? 'primary.contrastText' : 'text.primary' },
                  }}
                >
      ...
      <Box sx={{ p: collapsed && !isMobile ? 1 : 1.25, pt: 0 }}>
        <Button fullWidth onClick={() => navigate('/app/profile')} sx={{ justifyContent: collapsed && !isMobile ? 'center' : 'flex-start', px: 1.15, py: 1.05, color: 'text.primary', bgcolor: 'action.hover', '&:hover': { bgcolor: 'action.selected' } }}>
  ...
  return (
    <Box className="sa-app-main">
      <AppBar data-secureasset-app-header-theme="design-navigation-v160" position="fixed" elevation={0} sx={{ zIndex: theme.zIndex.drawer + 1, ml: { md: `${width}px` }, width: { md: `calc(100% - ${width}px)` }, bgcolor: design.colors.navigation, color: design.colors.navigationText, borderBottom: `${design.borders.width}px ${design.borders.style}`, borderColor: 'rgba(255,255,255,.20)', backdropFilter: design.effects.enableGlassNavigation ? 'blur(18px)' : 'none', transition: 'all .2s', '& .MuiIconButton-root': { color: design.colors.navigationText, bgcolor: 'rgba(255,255,255,.12)', '&:hover': { bgcolor: 'rgba(255,255,255,.20)' } }, '& .MuiButton-outlined': { color: design.colors.navigationText, borderColor: 'rgba(255,255,255,.46)', '&:hover': { bgcolor: 'rgba(255,255,255,.12)', borderColor: design.colors.navigationText } } }}>
        <Toolbar sx={{ minHeight: `${design.layout.appBarHeight}px !important`, px: { xs: 1.5, sm: 2.5, lg: 3.5 }, gap: { xs: .5, sm: 1.25 } }}>
          ...
          <IconButton onClick={(e) => setAnchor(e.currentTarget)} sx={{ ml: .15, p: .25, border: '2px solid', borderColor: 'background.paper', boxShadow: '0 0 0 1px', color: 'divider' }}><Avatar src={user?.avatar} sx={{ width: 35, height: 35, bgcolor: 'primary.main', fontSize: 14, fontWeight: 800 }}>{user?.name?.[0]}</Avatar></IconButton>
      ...
      <Drawer variant={isMobile ? 'temporary' : 'permanent'} open={isMobile ? mobileOpen : true} onClose={() => setMobileOpen(false)} ModalProps={{ keepMounted: true }} sx={{ '& .MuiDrawer-paper': { width: isMobile ? drawerWidth : width, boxSizing: 'border-box', borderRight: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', transition: 'width .2s', overflowX: 'hidden', boxShadow: 'none' } }}>{drawerContent}</Drawer>

      <Box component="main" sx={{ ml: { md: `${width}px` }, pt: `${design.layout.appBarHeight}px`, pb: { xs: 12, md: 5 }, minHeight: '100vh', transition: 'margin .2s' }}>
        <Box className="sa-app-content" sx={{ pt: { xs: 2, md: 3 } }}>
          <Outlet />
        </Box>
      </Box>

      <BottomNavigation className="sa-app-bottom-navigation sa-global-mobile-bottom-navigation" value={mobileBottomValue} onChange={(_e, value) => { const item = mobileBottomItems.find((candidate) => candidate.key === value); if (item) { navigate(item.path); setMobileOpen(false); } }} showLabels sx={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: theme.zIndex.appBar, border: 0, borderTop: `${design.borders.width}px ${design.borders.style}`, borderColor: 'divider', borderRadius: 0, overflow: 'hidden', bgcolor: design.bottomAppBar.background, opacity: design.bottomAppBar.opacity, pb: 'env(safe-area-inset-bottom)', backdropFilter: design.bottomAppBar.blur ? `blur(${design.bottomAppBar.blur}px)` : 'none', boxShadow: 'none', height: `calc(${design.bottomAppBar.height}px + env(safe-area-inset-bottom))`, '& .MuiBottomNavigationAction-root': { color: design.bottomAppBar.inactiveColor, minWidth: 0, px: 1, position: 'relative', '&.Mui-selected': { color: design.bottomAppBar.activeColor }, '&.Mui-selected::after': { content: '""', position: 'absolute', left: '28%', right: '28%', bottom: 0, height: 3, bgcolor: design.bottomAppBar.indicatorColor }, '& .MuiSvgIcon-root': { fontSize: design.bottomAppBar.iconSize }, '& .MuiBottomNavigationAction-label': { fontSize: design.bottomAppBar.labelSize, fontWeight: 760 } } }}>
      ...
      <Tooltip title={moduleError || 'Help & support'}><IconButton onClick={() => navigate('/contact')} sx={{ position: 'fixed', right: { xs: 20, md: 26 }, bottom: { xs: 94, md: 26 }, bgcolor: 'primary.main', color: 'primary.contrastText', border: '1px solid rgba(255,255,255,.30)', boxShadow: 'none', '&:hover': { bgcolor: 'primary.dark' } }}><HelpOutlineRounded fontSize="small" /></IconButton></Tooltip>
    </Box>
  );
}
