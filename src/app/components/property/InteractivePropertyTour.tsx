...
function RoomCard({ unit, active, onSelect }: { unit: any; active: boolean; onSelect: () => void }) {
  ...
  return <Paper
    ...
    sx={{
      ...
      borderRadius: 3, bgcolor: '#fff', p: 1, textAlign: 'left', cursor: 'pointer', color: 'inherit',
  ...
  >
    <Stack direction="row" spacing={1.15} alignItems="center">
      <Box sx={{ width: 78, height: 72, borderRadius: 2, overflow: 'hidden', flexShrink: 0 }}><OptimizedImage src={image} alt={unit.name || unit.roomNumber} width={180} height={150} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></Box>
...
export default function InteractivePropertyTour({ property, units, onBack, onBook, onShare, onToggleSaved, onView, saved }: TourProps) {
  ...
  return <Paper
    data-secureasset-interactive-tour="reference-clone-v202"
    elevation={0}
    sx={{ overflow: 'hidden', borderRadius: { xs: 0, sm: 4.5 }, border: { xs: 0, sm: '1px solid #E4EBF0' }, bgcolor: '#FBFDFE', boxShadow: { xs: 'none', sm: '0 24px 70px rgba(17, 52, 75, .1)' } }}
  >
    <Box sx={{ p: { xs: 1.6, sm: 2.5, lg: 3 } }}>
      ...
      <Stack direction="row" spacing={.8} sx={{ display: { xs: 'none', md: 'flex' }, mt: 2.2, overflowX: 'auto', pb: .35 }}>
        {FILTERS.map((filter) => <Chip key={filter.key} clickable onClick={() => setCategory(filter.key)} label={<>{filter.label} <Box component="span" sx={{ ml: .55, color: category === filter.key ? 'inherit' : '#8A98A8' }}>{filterCounts[filter.key] || 0}</Box></>} sx={{ height: 34, borderRadius: 2, bgcolor: category === filter.key ? '#008C73' : '#fff', color: category === filter.key ? '#fff' : '#345', border: '1px solid', borderColor: category === filter.key ? '#008C73' : '#E4EAF0', fontWeight: 800, boxShadow: category === filter.key ? '0 7px 18px rgba(0, 140, 115, .2)' : 'none' }} />)}
        <Box sx={{ flex: 1 }} />
      ...
      <Box sx={{ mt: { xs: 1.2, md: 1.7 }, display: { md: 'grid' }, gridTemplateColumns: { md: 'minmax(0, 1fr) 280px', lg: 'minmax(0, 1fr) 320px' }, gap: 1.6 }}>
        <Box sx={{ position: 'relative', height: { xs: 425, sm: 500, md: 520 }, borderRadius: { xs: 3, md: 3.5 }, overflow: 'hidden', bgcolor: '#DDE8EA' }}>
          ...
          {tab === 'tour' && HOTSPOT_LAYOUT.map((hotspot, index) => <Box key={hotspot.label} component="button" type="button" onClick={() => activateHotspot(index, hotspot)} onKeyDown={(event) => hotspotKey(event, index, hotspot)} aria-label={`Explore ${hotspot.label}`} sx={{ position: 'absolute', left: hotspot.left, top: hotspot.top, transform: 'translate(-50%,-50%)', border: 0, bgcolor: 'transparent', p: 0, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: .55, zIndex: 2 }}>
            ...
            <Typography sx={{ display: { xs: index === 4 ? 'block' : 'none', sm: 'block' }, bgcolor: 'rgba(9,31,43,.78)', px: .8, py: .4, borderRadius: 1, fontSize: { xs: 9, md: 11 }, fontWeight: 850, backdropFilter: 'blur(7px)', whiteSpace: 'nowrap' }}>{hotspot.label}</Typography>
          </Box>)}
          {tab === 'floor' && <Box sx={{ position: 'absolute', inset: 16, bgcolor: 'rgba(255,255,255,.94)', borderRadius: 3, display: 'grid', placeItems: 'center', textAlign: 'center', p: 3 }}><Box><MapOutlined sx={{ fontSize: 54, color: '#008C73' }} /><Typography fontWeight={950} color="#102A43">Floor plan overview</Typography><Typography color="text.secondary" fontSize={12}>Choose a room below to inspect its placement and dimensions.</Typography></Box></Box>}
          <Stack direction="row" spacing={.8} alignItems="center" sx={{ position: 'absolute', left: 14, bottom: 13, px: 1.1, py: .55, bgcolor: 'rgba(7,26,36,.76)', color: '#fff', borderRadius: 1.6, backdropFilter: 'blur(8px)' }}><LocationOnOutlined sx={{ fontSize: 15 }} /><Typography fontSize={10.5} fontWeight={750}>Click hotspots to explore the property</Typography></Stack>
        ...
        <Paper variant="outlined" sx={{ display: { xs: 'none', md: 'flex' }, flexDirection: 'column', borderRadius: 3.5, p: 2.2, borderColor: '#E2E9EE', bgcolor: '#fff' }}>
      ...
      <Box sx={{ display: { xs: 'block', md: 'none' }, position: 'relative', mt: -1.3, zIndex: 3 }}>
        {detailsOpen ? <Paper elevation={5} sx={{ mx: .5, p: 1.5, borderRadius: 3.2, bgcolor: '#fff' }}>
          <Stack direction="row" spacing={1.2} alignItems="flex-start"><Box sx={{ width: 82, height: 72, borderRadius: 2, overflow: 'hidden', flexShrink: 0 }}><OptimizedImage src={selectedImages[0]?.url || fallback} alt={selected.name || selected.roomNumber} width={190} height={160} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></Box><Box sx={{ flex: 1, minWidth: 0 }}><Stack direction="row" justifyContent="space-between"><Box><Typography noWrap fontWeight={950} fontSize={14}>{selected.name || `Room ${selected.roomNumber}`}</Typography><Chip size="small" label={status.label} sx={{ mt: .4, height: 21, bgcolor: status.available ? '#DDF7EC' : '#FFE5E5', color: status.available ? '#08785E' : '#C12D2D', fontSize: 9.5, fontWeight: 900 }} /></Box><IconButton size="small" onClick={() => setDetailsOpen(false)}><CloseRounded fontSize="small" /></IconButton></Stack><Typography fontWeight={950} fontSize={13} mt={.5}>{money(Number(selected.pricing?.monthlyRent || 0))} / month</Typography></Box></Stack>
          <Divider sx={{ my: 1.3 }} />
