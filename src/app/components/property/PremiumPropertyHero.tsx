    ...
    >
      <Box sx={{ p: { xs: 1.5, sm: 2.25, md: 3 } }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0, 1fr)', lg: 'minmax(0, 1fr) 315px' }, gap: { xs: 1.5, lg: 2.25 }, mt: { xs: 1.5, md: 2.25 } }}>
          <Box sx={{ minWidth: 0 }}>
            {tab === 'photos' ? <>
              <Box sx={{ height: mediaFrameHeight, minWidth: 0, minHeight: 0 }}>
                <Box sx={{ position: 'relative', width: '100%', height: '100%', minWidth: 0, minHeight: 0, overflow: 'hidden', borderRadius: HERO_RADIUS, bgcolor: '#E1EAED' }}>
                  ...
                  <Chip icon={<ImageRounded sx={{ fontSize: '16px !important' }} />} label={`${activeImage + 1} / ${gallery.length}`} size="small" sx={{ position: 'absolute', left: 13, top: 13, bgcolor: 'rgba(8,29,42,.80)', color: '#fff', fontWeight: 850, backdropFilter: 'blur(10px)', '& .MuiChip-icon': { color: '#fff' } }} />
                  {gallery.length > 1 && <Stack aria-label="Property photo slider" direction="row" spacing={.65} sx={{ position: 'absolute', left: 10, right: 10, bottom: 10, maxWidth: { sm: 'calc(100% - 20px)', md: '82%' }, overflowX: 'auto', p: .55, bgcolor: 'rgba(8,29,42,.72)', border: '1px solid rgba(255,255,255,.22)', borderRadius: HERO_RADIUS, backdropFilter: 'blur(12px)', scrollSnapType: 'x proximity', '&::-webkit-scrollbar': { display: 'none' } }}>
                    {gallery.map((image, index) => <Box component="button" type="button" key={`${image}-${index}`} onClick={() => setActiveImage(index)} aria-label={`View photo ${index + 1}`} aria-pressed={activeImage === index} sx={{ flex: { xs: '0 0 48px', sm: '0 0 58px' }, height: { xs: 36, sm: 42 }, p: 0, border: activeImage === index ? '2px solid #2BE4C3' : '1px solid rgba(255,255,255,.42)', borderRadius: HERO_RADIUS, overflow: 'hidden', cursor: 'pointer', bgcolor: '#E1EAED', opacity: activeImage === index ? 1 : .82, scrollSnapAlign: 'start', transition: 'opacity .2s ease, border-color .2s ease, transform .2s ease', '&:hover': { opacity: 1, transform: 'translateY(-1px)' } }}><OptimizedImage src={image} alt="" width={160} height={112} sizes="(max-width: 599px) 48px, 58px" style={{ objectFit: 'cover', objectPosition: 'center' }} /></Box>)}
                  </Stack>}
                </Box>
              </Box>
