  ...
  return <Box sx={{ maxWidth: 1440, mx: 'auto', px: { xs: 1.5, sm: 2.5 }, pt: 1.5, pb: 5 }}>
    <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1} sx={{ mb: 2.5 }}>
      <Stack direction="row" alignItems="center" spacing={1.3}>
        ...
        <Box><Typography component="h1" sx={{ fontSize: { xs: 21, md: 26 }, fontWeight: 500 }}>{propertyId ? 'Property tenancy history' : 'Tenancy History'}</Typography><Typography color="text.secondary" sx={{ fontSize: 12.5, mt: .4 }}>{propertyId ? 'Current tenants, previous stays and recorded activity.' : 'Choose one of your properties to review its complete tenancy record.'}</Typography></Box>
      </Stack>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={1.5} alignItems={{ md: 'center' }} sx={{ mb: 2 }}>
      {propertyId ? <Tabs value={filter} onChange={(_event, value) => { setFilter(value); setPage(1); }} variant="scrollable" scrollButtons="auto" aria-label="Filter tenancy history" sx={{ minHeight: 40, '& .MuiTab-root': { textTransform: 'none', minHeight: 40, fontSize: 12 } }}>{(Object.keys(groups) as HistoryFilter[]).filter((key) => key !== 'other' || counts.other).map((key) => <Tab value={key} key={key} label={groups[key]} />)}</Tabs> : <Typography color="text.secondary" sx={{ fontSize: 13 }}>{pagination ? `${pagination.total} properties` : 'Your properties'}</Typography>}
    ...
    {!history.isFetching && !history.isError && records.length === 0 && <Card variant="outlined" sx={{ textAlign: 'center', py: 5, px: 2, borderRadius: 3 }}><HomeWorkRounded sx={{ fontSize: 32, color: 'text.secondary', mb: 1 }} /><Typography sx={{ fontSize: 16, fontWeight: 500 }}>{propertyId ? 'No matching tenancy records' : 'No matching properties'}</Typography><Typography color="text.secondary" sx={{ fontSize: 13, mt: .5 }}>{query || filter !== 'all' ? 'Try another search or filter.' : propertyId ? 'Tenancy records will appear here as rental workflows are created.' : 'Properties you own will appear here, including those without a tenancy yet.'}</Typography></Card>}
