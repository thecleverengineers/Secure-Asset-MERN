...
import '../../../styles/security-premium.css';
...
export default function SecurityPage() {
  ...
  if (loading) return <Box className="sa-security-premium sa-security-loading"><CircularProgress /></Box>;

  return <Box className="sa-security-premium" sx={{ px: { xs: 2, sm: 3, lg: 4 }, pb: 6 }}>
    <PageHeader
      variant="plain"
      eyebrow="Account protection"
      title="Security Centre"
      description="Protect account access, trusted devices, verified contacts and your private Document Vault from one secure workspace."
      meta={<Stack direction="row" gap={.7} flexWrap="wrap" useFlexGap>
        <Chip size="small" variant="outlined" color={data.twoFactorEnabled ? 'success' : 'default'} label={data.twoFactorEnabled ? '2FA protected' : '2FA available'} />
        <Chip size="small" variant="outlined" color={data.deviceUnlockEnabled ? 'success' : 'default'} label={data.deviceUnlockEnabled ? 'Trusted device enabled' : 'Device unlock available'} />
      </Stack>}
      actions={<Button className="sa-security-refresh" size="small" startIcon={<RefreshRounded />} onClick={load}>Refresh</Button>}
    />
    ...
    <Grid className="sa-security-grid" container spacing={2}>
      <Grid size={{ xs: 12, lg: 7 }}>
        <Card className="sa-security-hero" elevation={0}>
